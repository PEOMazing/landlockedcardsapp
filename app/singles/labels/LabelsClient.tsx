...
type L = { id: string; cardNo: string; name: string; setName: string; number: string; condition: string; printing: string; comp: number | null; qr: string; location: string };
    ...
    (async () => {
      try {
        ...
        for (const id of ids) {
          ...
          out.push({ id, cardNo: formatCardNo(s.cardNo), name: clean(s.name), setName: s.setName, number: s.number, condition: s.condition, printing: s.printing, comp: s.comp, qr, location: s.location || "" });
        }
        setLabels(out);
      } catch {
  ...
  return (
    <>
      <style>{`
        ...
        .lbl img { width: 0.7in; height: 0.7in; }
        .cardno { font-weight: 800; font-size: 15pt; letter-spacing: 0.5px; line-height: 1; font-variant-numeric: tabular-nums; flex-shrink: 0; }
        @media print {
      ...
      <main className="py-6">
        <div className="no-print max-w-[8.5in] mx-auto mb-4 flex items-center justify-between px-2">
          <div>
            ...
            <div className="text-dim text-xs">The big number is the card&apos;s permanent ID - it shows on the stream line when the card is hit. Scan a label to open the quick-sell page. Load 1in x 2 5/8in label sheets and print at 100% scale.</div>
          </div>
          ...
          {labels.map((l) => (
            <div key={l.id} className="lbl">
              ...
              <div style={{ minWidth: 0, lineHeight: 1.15 }}>
                ...
                <div style={{ fontSize: "7pt" }}>{l.condition}{l.location ? <span style={{ marginLeft: 4 }}>{l.location}</span> : null}</div>
